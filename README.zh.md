...
当前发布固件的 Ubuntu UI 字体不含汉字字形，如需在设备上完整显示中文界面，需本地编译并在编译前执行以下步骤：

```bash
# 1. 下载 Noto Sans SC 字体（Regular + Bold）
#    https://fonts.google.com/noto/specimen/Noto+Sans+SC
#    将 NotoSansSC-Regular.ttf 和 NotoSansSC-Bold.ttf 放入：
#    lib/EpdFont/builtinFonts/source/NotoSansSC/

# 2. 安装 Python 依赖
pip install freetype-py fonttools

# 3. 生成 CJK UI 字体（只需运行一次）
...
生成的字体文件体积极小（337 个汉字子集，压缩后约 5–15 KB），对固件总体积影响可忽略不计。
